...
def generate_launch_description():
    ...
    moveit_shared_path = os.path.join(get_package_share_directory('mycobot_320pi_gz'))
    desc_path = os.path.join(desc_shared_path, 'urdf','original','mycobot_320_pi_moveit_2022_gz.urdf')
    ...
    moveit_simple_controllers_yaml = load_yaml("mycobot_320pi_gz", "config/ros2_controllers.yaml")
    moveit_controllers = {
    ...
    run_move_group_node = Node(
        ...
        parameters=[
            ...
            str(robot_description_kinematics),
            str(ompl_planning_pipeline_config),
            str(trajectory_execution),
            str(moveit_controllers),
            str(planning_scene_monitor_parameters),
        ],
    )

    # RVIZ:
    load_RVIZfile = LaunchConfiguration("rviz_file")
    rviz_base = os.path.join(get_package_share_directory("mycobot_320pi_gz"), "config")
