...
class Window: 
    def __init__(self, handle):
        ...
        # 夹爪开关按钮
        tk.Button(self.frmLB, text="gripper open", command=self.gripper_open, width=5).grid(
            row=1, column=0, sticky="w", padx=3, pady=50
        )
        tk.Button(self.frmLB, text="gripper close", command=self.gripper_close, width=5).grid(
